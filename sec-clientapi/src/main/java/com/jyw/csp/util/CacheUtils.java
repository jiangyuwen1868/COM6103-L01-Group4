...
public class CacheUtils {
	private static final Map<String, Class<?>> CLASS_CACHE = new ConcurrentHashMap<String, Class<?>>(1000);
	...
	public static Class<?> getClassForName(String className) {
		Class<?> targetClass = CLASS_CACHE.get(className);
		if (targetClass == null) {
			try {
				targetClass = forName(className, Thread.currentThread().getContextClassLoader());

				CLASS_CACHE.put(className, targetClass);
			} catch (Throwable t) {
				throw new CommonRuntimeException("XATF300100AA", "类名：" + className, t);
			}
		}
