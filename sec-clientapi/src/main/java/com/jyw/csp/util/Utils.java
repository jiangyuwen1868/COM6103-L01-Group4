...
import com.jyw.csp.api.exception.CommonRuntimeException;
...
//		String sTag = derValue.substring(offset, offset+length);
	 ...
	 * @return
	 */
	public static byte[] bytesXor(byte[] bytes1, byte[] bytes2) {
	...
	public static String XOR(String hexSrcData1, String hexSrcData2) {
		...
		byte[] bytes1 = Utils.hex2Byte(hexSrcData1);

		byte[] bytes2 = Utils.hex2Byte(hexSrcData2);
		...
		return Utils.bytes2Hex(buffer.array());
	}
	...
	public static String reversBytes(String hexSrcData)
	{
		...
		byte[] srcBytes = Utils.hex2Byte(hexSrcData);
		...
		return Utils.bytes2Hex(destBuffer.array());
	}
	
	public static byte[] paddingModel0(byte[] msg) {
		byte[] result = null;
		...
		if (tmp != 0) {
			result = new byte[len + (8-tmp)];
			for (int i = 0; i < 8 -tmp; i++) {
				if (i == 0) {
					result[len] = (byte) 0x80;
				} else {
					result[len + i] = (byte) 0x00;
				}
			}
		} else {
			return msg;
	...
	public static byte[] paddingModel1(byte[] msg) {
		byte[] result = null;
		...
		byte[] btmp = null;
		if (tmp != 0) {
			btmp = new byte[len + (8-tmp)];
			System.arraycopy(result, 0, btmp, 0, len);
			for (int i = 0; i < 8 - tmp; i++) {
				btmp[len + i] = (byte) 0x00;
			}
		} else {
			btmp = result;
		}
