...
//		}
	...
	public static String encryptC1C3C2(String publicKey, String data)
			throws IOException {
		// 国密规范正式公钥 04开头
		if (publicKey.length() == 128) {
			publicKey = "04" + publicKey;
		}
		return encryptC1C3C2(Utils.hex2Byte(publicKey), data.getBytes());
	}
	...
	public static byte[] decryptC1C3C2(String privateKey, String encryptedData)
			throws IOException {
		return decryptC1C3C2(Utils.hex2Byte(privateKey),
				Utils.hex2Byte(encryptedData));
	}
	...
	public static byte[] decryptC1C3C2(byte[] privateKey, byte[] encryptedData)
        ...
        byte[] c1 = Utils.hex2Byte(data.substring(0, 130));
	...
	public static String getPublicKey(String binaryKey) throws CertificateException, IOException, NoSuchProviderException {
		X509Certificate cert = SM2Util.getPemCert((binaryKey).getBytes(StandardCharsets.UTF_8));
		BCECPublicKey publicKey = (BCECPublicKey) cert.getPublicKey();
		String publicKeyString = Utils.bytes2Hex(publicKey.getEncoded());
		String pubKey = publicKeyString.substring(publicKeyString.length() - 128);
		return pubKey;

	}

	public static X509Certificate getPemCert(byte[] data) throws IOException, CertificateException, NoSuchProviderException {
		//使用BC，先remove的原因是有些系统的BC库会与添加的冲突
		Security.removeProvider("BC");
		Security.addProvider(new BouncyCastleProvider());
		//输出X509格式证书
		InputStream inStream = new ByteArrayInputStream(data);
		java.security.cert.CertificateFactory certificateFactory = java.security.cert.CertificateFactory.getInstance("X.509", "BC");//使用BC获取工厂实例
		X509Certificate x509Certificate = (X509Certificate) certificateFactory.generateCertificate(inStream);//用文件流读入证书
