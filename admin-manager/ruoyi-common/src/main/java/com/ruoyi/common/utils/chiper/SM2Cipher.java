...
public class SM2Cipher {
	...
	private void Reset() {
		...
		byte p[] = p2.normalize().getXCoord().getEncoded();
		//System.out.println("X:" + Utils.bytes2Hex(p));
		this.sm3keybase.update(p, 0, p.length);
		this.sm3c3.update(p, 0, p.length);

		//p = Utils.byteConvert32Bytes(p2.getY().toBigInteger());
		p = p2.normalize().getYCoord().getEncoded();
		//System.out.println("Y:" + Utils.bytes2Hex(p));
		this.sm3keybase.update(p, 0, p.length);
	...
	public ECPoint Init_enc(SM2Util.SM2 sm2, ECPoint userKey) {
	...
	public void Dofinal(byte c3[]) {
		...
		byte p[] = p2.normalize().getYCoord().getEncoded();
		this.sm3c3.update(p, 0, p.length);
